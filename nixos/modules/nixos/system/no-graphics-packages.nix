...
with lib;
{
  options.mySystem.system.noGraphicsPackages = lib.mkOption {
    type = lib.types.bool;
    default = false;
    description = ''
  ...
  config = lib.mkIf cfg.noGraphicsPackages {
    assertions = [
      {
        ...
        message = "mySystem.system.noGraphicsPackagess requires that no graphical desktop is being used! Please unset this option.";
      }
    ];
