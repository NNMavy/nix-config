# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  adguard-exporter = {
    pname = "adguard-exporter";
    version = "v1.2.1";
    src = fetchFromGitHub {
      owner = "henrywhitaker3";
      repo = "adguard-exporter";
      rev = "v1.2.1";
      fetchSubmodules = false;
      sha256 = "sha256-OltYzxBOOcaW3oYNFvxxjG1qRvuLaZfReSeQaNGiRDc=";
    };
  };
  chrony-exporter = {
    pname = "chrony-exporter";
    version = "v0.12.1";
    src = fetchFromGitHub {
      owner = "SuperQ";
      repo = "chrony_exporter";
      rev = "v0.12.1";
      fetchSubmodules = false;
      sha256 = "sha256-S5Mox5HmOJ+PiV6aq/tqubf6kfOdOMvvUk+m0VzOdl0=";
    };
  };
  cockpit-podman = {
    pname = "cockpit-podman";
    version = "428bcd271d692472e47a8fdc6e30be96e68bc0f1";
    src = fetchFromGitHub {
      owner = "cockpit-project";
      repo = "cockpit-podman";
      rev = "428bcd271d692472e47a8fdc6e30be96e68bc0f1";
      fetchSubmodules = false;
      sha256 = "sha256-UPtmECJ84jxEeWPqRzdaT5SH4pqA4jKgHpKKKv1uQUg=";
    };
    date = "2025-10-27";
  };
  cosmic-idle = {
    pname = "cosmic-idle";
    version = "267bb837f127eb805a17250ebcad02db57eb72cb";
    src = fetchFromGitHub {
      owner = "pop-os";
      repo = "cosmic-idle";
      rev = "267bb837f127eb805a17250ebcad02db57eb72cb";
      fetchSubmodules = false;
      sha256 = "sha256-dRvcow+rZ4sJV6pBxRIw6SCmU3aXP9uVKtFEJ9vozzI=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./cosmic-idle-267bb837f127eb805a17250ebcad02db57eb72cb/Cargo.lock;
      outputHashes = {
        "clipboard_macos-0.1.0" = "sha256-tovB4fjPVVRY8LKn5albMzskFQ+1W5ul4jT5RXx9gKE=";
        "atomicwrites-0.4.2" = "sha256-QZSuGPrJXh+svMeFWqAXoqZQxLq/WfIiamqvjJNVhxA=";
        "cosmic-settings-config-0.1.0" = "sha256-BCOVyJ1IIik/R4qC/16csJH8yII4WxdxO116hdvUl3I=";
        "cosmic-config-0.1.0" = "sha256-O7QVpdKVUw32TOxns+ozP6b/E8eh/jE007Ki63seBiY=";
        "smithay-clipboard-0.8.0" = "sha256-4InFXm0ahrqFrtNLeqIuE3yeOpxKZJZx+Bc0yQDtv34=";
        "upower_dbus-0.3.2" = "sha256-9Pq5WFBeIRvP2VZaa3BzoqiQmzN6taa20u7k+2aF3v0=";
      };
    };
    date = "2025-02-25";
  };
  flux-local = {
    pname = "flux-local";
    version = "7.11.0";
    src = fetchurl {
      url = "https://pypi.org/packages/source/f/flux_local/flux_local-7.11.0.tar.gz";
      sha256 = "sha256-dh+Con+8Um/wwBaHeACDPWRsRCNAJOslUJ/C0WfYuVU=";
    };
  };
  gpsd-prometheus-exporter = {
    pname = "gpsd-prometheus-exporter";
    version = "6a6009c8f89874eef46690abd96cf6f1d1149d9f";
    src = fetchFromGitHub {
      owner = "brendanbank";
      repo = "gpsd-prometheus-exporter";
      rev = "6a6009c8f89874eef46690abd96cf6f1d1149d9f";
      fetchSubmodules = false;
      sha256 = "sha256-oR+JNwf+QqWzreL0qzsxOqMCWZxFma4zREnAcToGkdY=";
    };
    date = "2025-08-09";
  };
  mqtt-explorer = {
    pname = "mqtt-explorer";
    version = "v0.3.5";
    src = fetchFromGitHub {
      owner = "thomasnordquist";
      repo = "MQTT-Explorer";
      rev = "v0.3.5";
      fetchSubmodules = false;
      sha256 = "sha256-lXHPKOZh5gbG+kpH9wLBcHztS7fBj9D57ySKd1xkhxc=";
    };
  };
  npiperelay = {
    pname = "npiperelay";
    version = "v0.1.0";
    src = fetchFromGitHub {
      owner = "jstarks";
      repo = "npiperelay";
      rev = "v0.1.0";
      fetchSubmodules = false;
      sha256 = "sha256-cg4aZmpTysc8m1euxIO2XPv8OMnBk1DwhFcuIFHF/1o=";
    };
  };
  talosctl = {
    pname = "talosctl";
    version = "v1.11.3";
    src = fetchFromGitHub {
      owner = "siderolabs";
      repo = "talos";
      rev = "v1.11.3";
      fetchSubmodules = false;
      sha256 = "sha256-6nm0KgENOHUdyJllvnhBNlxDGL3G8gg4KqhTkTRE32o=";
    };
  };
}
