# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  adguard-exporter = {
    pname = "adguard-exporter";
    version = "v1.2.0";
    src = fetchFromGitHub {
      owner = "henrywhitaker3";
      repo = "adguard-exporter";
      rev = "v1.2.0";
      fetchSubmodules = false;
      sha256 = "sha256-HQ72eTBV6ZZMawACZ/iwABVm8On/yqFKotfXxbGZqUc=";
    };
  };
  chrony-exporter = {
    pname = "chrony-exporter";
    version = "v0.12.1";
    src = fetchFromGitHub {
      owner = "SuperQ";
      repo = "chrony_exporter";
      rev = "v0.12.1";
      fetchSubmodules = false;
      sha256 = "sha256-S5Mox5HmOJ+PiV6aq/tqubf6kfOdOMvvUk+m0VzOdl0=";
    };
  };
  cockpit-podman = {
    pname = "cockpit-podman";
    version = "6be070a67baa939731db08ddb802777930632399";
    src = fetchFromGitHub {
      owner = "cockpit-project";
      repo = "cockpit-podman";
      rev = "6be070a67baa939731db08ddb802777930632399";
      fetchSubmodules = false;
      sha256 = "sha256-hTRl9l2byRn4sX87BR9xB+xsXRRqHQXm/pA8nqn4uLc=";
    };
    date = "2025-08-04";
  };
  cosmic-idle = {
    pname = "cosmic-idle";
    version = "267bb837f127eb805a17250ebcad02db57eb72cb";
    src = fetchFromGitHub {
      owner = "pop-os";
      repo = "cosmic-idle";
      rev = "267bb837f127eb805a17250ebcad02db57eb72cb";
      fetchSubmodules = false;
      sha256 = "sha256-dRvcow+rZ4sJV6pBxRIw6SCmU3aXP9uVKtFEJ9vozzI=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./cosmic-idle-267bb837f127eb805a17250ebcad02db57eb72cb/Cargo.lock;
      outputHashes = {
        "clipboard_macos-0.1.0" = "sha256-tovB4fjPVVRY8LKn5albMzskFQ+1W5ul4jT5RXx9gKE=";
        "atomicwrites-0.4.2" = "sha256-QZSuGPrJXh+svMeFWqAXoqZQxLq/WfIiamqvjJNVhxA=";
        "cosmic-settings-config-0.1.0" = "sha256-BCOVyJ1IIik/R4qC/16csJH8yII4WxdxO116hdvUl3I=";
        "cosmic-config-0.1.0" = "sha256-O7QVpdKVUw32TOxns+ozP6b/E8eh/jE007Ki63seBiY=";
        "smithay-clipboard-0.8.0" = "sha256-4InFXm0ahrqFrtNLeqIuE3yeOpxKZJZx+Bc0yQDtv34=";
        "upower_dbus-0.3.2" = "sha256-9Pq5WFBeIRvP2VZaa3BzoqiQmzN6taa20u7k+2aF3v0=";
      };
    };
    date = "2025-02-25";
  };
  flux-local = {
    pname = "flux-local";
    version = "7.7.1";
    src = fetchurl {
      url = "https://pypi.org/packages/source/f/flux_local/flux_local-7.7.1.tar.gz";
      sha256 = "sha256-2pFlwGutu4aWMh/3h0YTOAW11gJusdNYLkr0p7zk97I=";
    };
  };
  gpsd-prometheus-exporter = {
    pname = "gpsd-prometheus-exporter";
    version = "a395c6ffe5d6383d6c6d58e4b77c8c803f366180";
    src = fetchFromGitHub {
      owner = "brendanbank";
      repo = "gpsd-prometheus-exporter";
      rev = "a395c6ffe5d6383d6c6d58e4b77c8c803f366180";
      fetchSubmodules = false;
      sha256 = "sha256-w8xWluJ8drE7YYnRqg+0T/BjTTJeGdQChg+OWYStBFk=";
    };
    date = "2025-06-24";
  };
  mqtt-explorer = {
    pname = "mqtt-explorer";
    version = "v0.3.5";
    src = fetchFromGitHub {
      owner = "thomasnordquist";
      repo = "MQTT-Explorer";
      rev = "v0.3.5";
      fetchSubmodules = false;
      sha256 = "sha256-lXHPKOZh5gbG+kpH9wLBcHztS7fBj9D57ySKd1xkhxc=";
    };
  };
  npiperelay = {
    pname = "npiperelay";
    version = "v0.1.0";
    src = fetchFromGitHub {
      owner = "jstarks";
      repo = "npiperelay";
      rev = "v0.1.0";
      fetchSubmodules = false;
      sha256 = "sha256-cg4aZmpTysc8m1euxIO2XPv8OMnBk1DwhFcuIFHF/1o=";
    };
  };
  talosctl = {
    pname = "talosctl";
    version = "v1.10.6";
    src = fetchFromGitHub {
      owner = "siderolabs";
      repo = "talos";
      rev = "v1.10.6";
      fetchSubmodules = false;
      sha256 = "sha256-s1d1TVBGi9AZD/nXSv7graKgD6H1MLr4Fvg64c88A4A=";
    };
  };
}
